  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4 px-6 flex-shrink-0">
        <div>
          <h2 className="text-2xl font-semibold">Council</h2>
          <p className="text-sm text-muted-foreground">
            Multi-agent deliberation and decision making
          </p>
        </div>
        <Badge variant="default" className="gap-1.5">
          <Users2 className="h-3 w-3" />
          {debates.filter(d => d.status === 'active').length} active
        </Badge>
      </div>
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    ...
                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${(debate.votes.optionA / (debate.votes.optionA + debate.votes.optionB)) * 100}%` }}
                      />
                    </div>
                    <span className="text-xs text-muted-foreground">
                      {debate.votes.optionA}:{debate.votes.optionB}
                    </span>
                  </div>
                )}
              </CardContent>
